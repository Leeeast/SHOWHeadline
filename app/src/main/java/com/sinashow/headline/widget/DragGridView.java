...
public class DragGridView extends GridView {
    ...
    /* 最后一个不可移动ITEM的位置 */
    private static final int LAST_FIXED_ITEM_POS = -1;

    private boolean mFastDrag;
    ...
    public void init(Context context){
        ...
        mHorizontalSpacing =  (int) (mHorizontalSpacing * scale + 0.5f);
    ...
    public void setDragMode(boolean fastDrag) {
        mFastDrag = fastDrag;
    }
...
//		}
           ...
           prepareDrag(-1, ev);
            ...
            setOnItemLongClickListener(new OnItemLongClickListener() {
                ...
                public boolean onItemLongClick(AdapterView<?> parent, View view,
                    ...
                    return mFastDrag ? false : prepareDrag(position, ev);
                }
    ...
    protected boolean prepareDrag(int position, MotionEvent ev) {
        ...
        if (startPosition <= LAST_FIXED_ITEM_POS) {
            return false;
        }
        View dragView = getChildAt(dragPosition - getFirstVisiblePosition());
        TextView dragTextView = (TextView) dragView;
        ...
        if (dragPosition != AdapterView.INVALID_POSITION) {
            ...
            win_view_x = windowX - dragView.getLeft();//VIEW相对自己的X，半斤
            win_view_y = windowY - dragView.getTop();//VIEW相对自己的y，半斤
            dragOffsetX = (int) (ev.getRawX() - x);//手指在屏幕的上X位置-手指在控件中的位置就是距离最左边的距离
            dragOffsetY = (int) (ev.getRawY() - y);//手指在屏幕的上y位置-手指在控件中的位置就是距离最上边的距离
            dragItemView = dragView;
            ...
            if (getContext().checkCallingPermission(Manifest.permission.VIBRATE)
                    == PackageManager.PERMISSION_GRANTED) {
                mVibrator.vibrate(50);//设置震动时间
            }
            startDrag(dragBitmap, (int) ev.getRawX(), (int) ev.getRawY());
            hideDropItem();
            dragTextView.setText(null);
            dragTextView.setBackgroundResource(R.drawable.bg_channel_dash);
            isMoving = false;
...
//		this.windowParams.y = (y - this.win_view_y + this.viewY);//位置的y值
    ...
    public void onMove(int x, int y) {
        ...
        // 判断下方的POSTION是否是最开始N个不能拖动的
        if (dPosition > LAST_FIXED_ITEM_POS) {
            ...
            if (dPosition != dragPosition) {
                ...
                for (int i = 0; i < movecount_abs; i++) {
                    ...
                    View moveView = getChildAt(holdPosition);
                    Animation moveAnimation = getMoveAnimation(to_x, to_y);
                    moveView.startAnimation(moveAnimation);
